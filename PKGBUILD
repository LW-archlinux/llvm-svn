...
build() {
    ...
    cmake -G Ninja \
        -DCMAKE_SOURCE_DIR="$srcdir/$_pkgname" \
        -DCMAKE_BINARY_DIR="$srcdir"/_build
...
check() {
    ...
    [[ "${CARCH}" == "i686" ]] || LD_LIBRARY_PATH="${srcdir}/_build/lib" ninja -C "$srcdir"/_build check
    ninja -C "$srcdir"/_build check-clang
    ninja -C "$srcdir"/_build check-polly
}
...
package_clang-svn() {
    ...
    cd "${srcdir}/_build/tools/clang"

    # Exclude the extra directory, since it'll be installed in a separate package
    sed -i \
        "s|^\([[:blank:]]*include(\"${srcdir}/_build/tools/clang/tools/extra/cmake_install.cmake\")\)$|#\1|" \
...
package_clang-compiler-rt-svn() {
    ...
    cd "${srcdir}/_build/projects/compiler-rt"
...
package_clang-tools-extra-svn() {
    ...
    cd "${srcdir}/_build/tools/clang/tools/extra"
